...
- **Detects** which abilities are **ready** vs **on cooldown** by comparing brightness to calibrated baselines
- **Shows** a transparent overlay so you can align the capture region with your action bar
- **Displays** live slot states (green = ready, red = on cooldown) with optional countdown
- **Automation**: when enabled, presses keys in a **priority order**—only the first *ready* ability in the list is triggered, with a minimum delay between keypresses and an optional “target window” check so keys are only sent when the game has focus
...
5. **Start Capture** — Click **Start Capture**. The **Live Preview** and **Slot States** will update in real time. Each slot shows its keybind and state (ready or cooldown).

### Slot States (Left Panel)

Each slot is a button showing `[key]` and, when on cooldown, the remaining time.
...
- **Detection** — **Darken** and **Trigger** control how much brightness drop counts as “on cooldown” and what fraction of a slot must be darkened. Adjust if detection is too sensitive or not sensitive enough.
